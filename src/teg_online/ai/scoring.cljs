...
(defn borders [game countries]
  (->> countries
       (mapcat (fn [country]
                 (->> (-> b/countries country :neighbours)
                      (remove (fn [neighbour]
                                (= (teg/country-owner game country)
                                   (teg/country-owner game neighbour))))
                      (map (fn [neighbour]
                             [country neighbour])))))))

(defn border-strengh [army enemy-army]
  (/ (- army enemy-army)
     (+ army enemy-army)))
...
(comment
  ...
  (def empire (->> (player-empires game player-id)
                   (sort-by count >)
                   (first)))
